...
PeterPaul.chem.nutrients %>%
  ...
  ggplot(aes(
    x = daynum,
    y = temperature_C,
    color = year4 #Color your points by year
  )) +
  geom_point() +
  facet_wrap(vars(lakename), nrow=2) # facet by lake in two rows.
...
PeterPaul.chem.nutrients %>%
  ggplot(aes(
    x = sampledate,
    y = temperature_C, #Plot temperature by date
    color = depth
  )) + 
  geom_point(size = 0.5) +  # Change the size of your point to 0.5
  scale_color_viridis(option = 'magma', direction = -1) + #Color your points by depth 
  # Change the color palette to magma and play with direction (+- 1)
  scale_x_date(
    date_labels = '%Y',
    date_breaks = "5 years") 
...
# Make the points 50 % transparent #alpha
# Add a line of best fit for the linear regression of these variables. #geom_smooth
