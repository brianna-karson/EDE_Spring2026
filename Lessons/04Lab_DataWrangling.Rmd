...
glimpse(NTL.phys.data$sampledate) #exact same output

# Change sampledate values into date objects
NTL.phys.data$sampledate <- mdy(NTL.phys.data$sampledate) 
#always convert date in the same chunk as you read your data set
```
...
#c does not denote "condition" it is simply vector notation
```
...
> Answer: The row numbers are internal indexes. 
...
NTL.phys.data.gathered.irradiance <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck,
