...
un_subset <- st_read(here('./Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == "Upper Neuse") #Filter for just this sub basin

#Create a map
mapView(huc8_sf, col.regions = "Grey") +
mapView(un_subset,  fill = 'yellow', color = 'blue')
...
wind_energy <- read.csv(file = 'https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
...
##if you do the join the other way around it will drop the geometry - order matters

#View the outputs
HUC_8wind %>% ggplot + geom_sf(aes(fill=SumWindEnergy))
mapview(HUC_8_wind
