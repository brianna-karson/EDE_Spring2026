...
##date format column is necessary - must mutate columns to get this 
...
summary(BR_complete) ##but won't show NA if the whole row is missing
check_date <- data.frame("Date" = seq.Date(from = first(BR_complete$Date), 
                                           to = last(BR_complete$Date),
                                           by = "month")) 
##number of dates matches number of Rows in BR_complete 
##if they didn't match, join the new data frame to the BR_complete to check for NA values
...
rainfall_plot <- BR_complete %>%
  ggplot(aes(x = Date, y = Rainfall)) + 
  geom_line() +
  geom_smooth()
print(rainfall_plot)

##can tell its seasonal because it goes up and down over time at regular intervals
##blue trend line appears stationary - no real visible trend but will do analysis to check statistical significance
...
##still seasonal, but seeing an upward trend over time, consistent with climate change
...
temp.ts <- ts(BR_complete$Temperature_C, start = c(1901, 1), frequency = 12)
...
rainfall_decomp <- stl(rainfall.ts,s.window = "periodic")
plot(rainfall_decomp)

temp_decomp <- stl(temp.ts, s.window = "periodic") ##always periodic for stl function 
plot(temp_decomp)

##bar on the very right indicates how much you have to "zoom" to detect a pattern; smaller bar = covers a greater span of the data 
##bottom series "remainder" should be random with no seasonality; if that's not there, it means your approach was not appropriate for the data
